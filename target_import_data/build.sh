...
# 日志文件
log_path="${base_path}/target_import_data.log"
# 上次同步时间的记录文件
...
## 构建镜像并启动容器
docker build -t target_import_data_image .
docker run -d --name target_import_data \
           -v ${config_path}:/code/config.ini \
           -v ${log_path}:/code/target_import_data.log \
           ...
           target_import_data_image:latest
docker update target_import_data --restart=always